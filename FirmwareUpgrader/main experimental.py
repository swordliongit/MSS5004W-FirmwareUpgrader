...
class MasterGui(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.frame_1.grid(row=0, column=0, padx=20, pady=20, sticky="new")
        self.frame_1.grid_columnconfigure(0, weight=0)  # First column
        self.frame_1.grid_columnconfigure(1, weight=1)  # Second column
        self.frame_1.grid_columnconfigure(2, weight=0)  # Second column
        # self.frame_1.grid_columnconfigure(0, weight=0)

        self.ModemCountEntry = ctk.CTkEntry(master=self.frame_1, placeholder_text="Modem Sayısı")
        self.ModemCountEntry.grid(row=0, column=1, padx=10, pady=10)
        ...
        self.button_start.grid(row=1, column=1, pady=10, padx=10)
        # self.Console_label = ctk.CTkLabel(
        #     self.frame_1,
        #     text="Güncelleme Logları",
        #     anchor="w",
        # )
        # self.Console_label.grid(row=3, column=0, columnspan=3, padx=0, pady=0, sticky="ns")
        self.scrollable_frame = ctk.CTkScrollableFrame(self, label_text="Güncelleme Logları")
        self.scrollable_frame.grid(row=1, column=0, padx=10, pady=0, sticky="new")
        self.scrollable_frame.grid_columnconfigure(0, weight=1)

        self.Console = ctk.CTkTextbox(master=self.scrollable_frame)
        self.Console.grid(row=0, column=0, pady=10, padx=10, sticky="nsew")
        # self.Console.insert(ctk.END, " " * 55 + "Güncelleme Logları" + "\n" + "-" * 142 + "\n\n")
        self.Console.configure(state="disabled")

        self.tabview = ctk.CTkTabview(self, height=480, width=300)
        self.tabview.grid(row=0, column=1, padx=5, pady=0, sticky="n")
        ...
        self.texbox_about = ctk.CTkTextbox(
            master=self.tabview.tab("Hakkında"), width=280, height=400
        )
        ...
        self.texbox_mac_list = ctk.CTkTextbox(
            master=self.tabview.tab("Ayarlar"), width=280, height=300
        )
        self.texbox_mac_list.grid(row=3, column=0, pady=(55, 0))
        self.texbox_mac_list.insert(
            ctk.END, " " * 10 + "Tamamlanmış Güncelleme Listesi" + "\n" + "-" * 76 + "\n\n"
        )
    ...
    def Show_Update_Popup(self):
        showinfo(
            "Firmware Upgrader",
            "Yeni Güncelleme Mevcut!\nAyarlar->Güncellemeyi İndir butonundan güncellemeyi indirin",
        )
    ...
    def Update_Control(self):
        ...
        if INTERNET_CONNECTED:
            ...
            showinfo(
                "Firmware Upgrader", "Lütfen İnternet Bağlantınızı Kontrol Edip Tekrar Deneyin"
            )
